...
import { SimonSlide } from '../games/SimonSlide';
import { ColorMatch } from '../games/ColorMatch';
import { PitchGuess } from '../games/PitchGuess';
...
const GAMES = [
  ...
  { id: 'rhythm',   name: 'Ritim',       emoji: '🎵', desc: '8 vuruş · isabet = jeton' },
  { id: 'simon',    name: 'Simon',       emoji: '🟦', desc: 'renk dizisini hatırla' },
  { id: 'color',    name: 'Renk Tahmini',emoji: '🎨', desc: '5sn bak · en yakını seç' },
  { id: 'pitch',    name: 'Frekans',     emoji: '🎚', desc: 'dalgayı hisset · tahmin et' },
];

type GameId = 'stars' | 'reaction' | 'memory' | 'balon' | 'mat' | 'rhythm' | 'simon' | 'color' | 'pitch' | null;
...
export function PlaygroundActivity({ onBack }: { onBack: () => void }) {
  ...
  const launch = (id: GameId) => { sendFace('play'); setActiveGame(id); };
  ...
  if (activeGame === 'rhythm')   return <RhythmTap    onBack={() => setActiveGame(null)} />;
  if (activeGame === 'simon')    return <SimonSlide   onBack={() => setActiveGame(null)} />;
  if (activeGame === 'color')    return <ColorMatch   onBack={() => setActiveGame(null)} />;
  if (activeGame === 'pitch')    return <PitchGuess   onBack={() => setActiveGame(null)} />;
  ...
  return (
    <ActivityFrame scene="playground" title="Oyun Alanı"
      ...
      <View style={styles.list}>
        {GAMES.map(g => (
          <TouchableOpacity key={g.id} onPress={() => launch(g.id as GameId)}
